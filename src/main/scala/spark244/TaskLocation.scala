...
/**
 * A location on a host that will not be elevated to rack or any
 */
private [spark] case class FixedHostTaskLocation(override val host: String) extends TaskLocation {
  override def toString: String = host
}
...
private[spark] object TaskLocation {
  ...
  // Do not allow location to be anything else
  val fixedLocationTag = "*"
   ...
   * These strings have the form [*hostname], executor_[hostname]_[executorid], [hostname], or
  ...
  def apply(str: String): TaskLocation = {
    ...
    val fixed = str.stripPrefix(fixedLocationTag)
    if (!fixed.equals(str)) {
      return new FixedHostTaskLocation(fixed)
    }

    val hostAndExecutorId = str.stripPrefix(executorLocationTag)
    if (!hostAndExecutorId.equals(str)) {      
