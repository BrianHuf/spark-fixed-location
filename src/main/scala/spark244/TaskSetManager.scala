...
private[spark] class TaskSetManager(
  ...
  private[scheduler] var maxTaskLocality = TaskLocality.ANY
  ...
  private[spark] def addPendingTask(index: Int) {
    for (loc <- tasks(index).preferredLocations) {
      loc match {        
        ...
        case e: FixedHostTaskLocation =>           
          maxTaskLocality = TaskLocality.NODE_LOCAL
        case _ =>
      }
  ...
  def resourceOffer(
      execId: String,
      host: String,
      maxLocalityOffered: TaskLocality.TaskLocality)
    : Option[TaskDescription] =
  {
    val maxLocality = if (maxTaskLocality < maxLocalityOffered) maxTaskLocality else maxLocalityOffered
